...
<div class="tabHeader">
	...
	<c:forEach var="jobListTabValue" items="${jobListTabValues}">
		...
		<div class="jobListHeaderTab block-border${isSelectedTab ? " block-background" : ""}">
			...
			<a href="${eco:baseAdminUrlWithPrefix()}/jobs/${jobListTabValue.key}/">${jobListTabValue.nameTranslated}</a>

			<br />

			<c:if test="${activeTabAndThereAreRunningJobs}">
				<a href="${eco:baseAdminUrlWithPrefix()}/jobs/${jobListTabValue.key}/status/${activeStatusId}/" title="${eco:translate('Show active jobs')}">&nbsp;${activeJobsQty}&nbsp;</a> /
			</c:if>
