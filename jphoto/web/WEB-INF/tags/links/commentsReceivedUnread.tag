...
<c:set var="link" value="<%=ApplicationContextHelper.getUrlUtilsService().getReceivedUnreadComments( user.getId() )%>" />

<c:set var="userNameEscaped" value="${eco:escapeHtml(user.name)}" />
<c:set var="linkNerdText" value="<%=LinkNerdText.USER_STATISTICS_COMMENTS_RECEIVED_UNREAD%>"/>
