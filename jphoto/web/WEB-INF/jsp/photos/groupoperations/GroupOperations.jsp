		...
		<form:form modelAttribute="photoGroupOperationModel" action="${eco:baseUrlWithPrefix()}/photos/groupOperations/confirm/">
			...
			<c:if test="${photoGroupOperationType == 'ARRANGE_PHOTO_ALBUMS'}">
				...
				<c:forEach var="userPhotoAlbum" items="${photoGroupOperationModel.userPhotoAlbums}">
					<js:checkBoxChecker namePrefix="_${userPhotoAlbum.id}']" uniqueId="${userPhotoAlbum.hashCode}" /> ${userPhotoAlbum.name}
					<br />
				</c:forEach>
			</c:if>
			...
			<c:if test="${photoGroupOperationType == 'ARRANGE_NUDE_CONTENT'}">
				<js:checkBoxChecker namePrefix="_1']" uniqueId="1" /> ${eco:translate('Nude content')}
			</c:if>
