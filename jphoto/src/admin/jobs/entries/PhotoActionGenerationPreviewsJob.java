...
public class PhotoActionGenerationPreviewsJob extends AbstractPhotoActionGenerationJob {
	...
	public boolean doPhotoAction( final Photo photo, final User user ) {
		...
		addJobExecutionFinalMessage( String.format( "User %s has seen photo %s ( time: %s )", entityLinkUtilsService.getUserCardLink( user ), entityLinkUtilsService.getPhotoCardLink( photo, getLanguage() ), dateUtilsService.formatDateTime( actionTime ) ) );
