...
public enum ConfigurationKey {
	...
	, CANDIDATES_CAN_VOTE_FOR_USER_RANKS_IN_GENRE( 309, "0", false, ConfigurationDataType.YES_NO, ConfigurationUnit.EMPTY, ConfigurationTab.CANDIDATES, "Candidates can vote for ranks in genres of other members" )
	...
	, PHOTO_UPLOAD_ADDITIONAL_SIZE_WEEKLY_LIMIT_PER_RANK_KB( 401, "100", false, ConfigurationDataType.INTEGER, ConfigurationUnit.KILOBYTE, ConfigurationTab.PHOTO_UPLOAD, "Additional summary photos size for file uploading for each rank in genre per week" )
	...
	, COMMENTS_USER_MUST_HAVE_N_APPROVED_PHOTOS_BEFORE_CAN_LEAVE_COMMENT( 703, "2", false, ConfigurationDataType.INTEGER, ConfigurationUnit.PHOTO, ConfigurationTab.COMMENTS, "Member must have N approved by admin photos before ability to comment of photos" )

	, PHOTO_VOTING_HIGHEST_POSITIVE_MARK( 800, "5", true, ConfigurationDataType.INTEGER, ConfigurationUnit.MARK, ConfigurationTab.PHOTO_VOTING, "The highest accessible positive mark even if a voting member's own rank in genre is higher (cutting the highest mark if rank is too hight)" )
	, PHOTO_VOTING_LOWEST_POSITIVE_MARK( 801, "3", true, ConfigurationDataType.INTEGER, ConfigurationUnit.MARK, ConfigurationTab.PHOTO_VOTING, "The highest accessible positive mark even if a voting member's own rank in genre is lower (adding an extra positive balls to the highest mark if rank is too low)" )
	, PHOTO_VOTING_HIGHEST_NEGATIVE_MARK( 802, "-2", true, ConfigurationDataType.INTEGER, ConfigurationUnit.MARK, ConfigurationTab.PHOTO_VOTING, "The lowest accessible negative mark even if a voting member's own rank in genre is lower (adding an extra negative balls to the lowest mark if rank is too low)" )
	, PHOTO_VOTING_LOWEST_NEGATIVE_MARK( 803, "-3", true, ConfigurationDataType.INTEGER, ConfigurationUnit.MARK, ConfigurationTab.PHOTO_VOTING, "The lowest accessible negative mark even if a voting member's own rank in genre is higher (cutting the lowest mark if rank is too hight)" )
	...
	, CACHE_LENGTH_GENRE( 903, "30", false, ConfigurationDataType.INTEGER, ConfigurationUnit.ITEM, ConfigurationTab.CACHE, "Genre cache length" )
	, CACHE_LENGTH_GENRE_VOTING_CATEGORY( 904, "30", false, ConfigurationDataType.INTEGER, ConfigurationUnit.ITEM, ConfigurationTab.CACHE, "Genre voting category cache length" )
	...
	, CACHE_LENGTH_USER_GENRE_RANK( 911, "1111", false, ConfigurationDataType.INTEGER, ConfigurationUnit.ITEM, ConfigurationTab.CACHE, "User rank in genre cache length" )
	, CACHE_LENGTH_RANK_IN_GENRE_POINTS( 912, "200", false, ConfigurationDataType.INTEGER, ConfigurationUnit.ITEM, ConfigurationTab.CACHE, "Rank in genre points cache length" )
	, CACHE_LENGTH_USER_PHOTOS_BY_GENRES( 913, "2000", false, ConfigurationDataType.INTEGER, ConfigurationUnit.ITEM, ConfigurationTab.CACHE, "User photos in genre quantity cache length" )

	, PHOTO_CARD_MAX_WIDTH( 1010, "1100", false, ConfigurationDataType.INTEGER, ConfigurationUnit.PIXEL, ConfigurationTab.PHOTO_CARD, "Photo card image max width" )
	, PHOTO_CARD_MAX_HEIGHT( 1011, "900", false, ConfigurationDataType.INTEGER, ConfigurationUnit.PIXEL, ConfigurationTab.PHOTO_CARD, "Photo card image max height" )
