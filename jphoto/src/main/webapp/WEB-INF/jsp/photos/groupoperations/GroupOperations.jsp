<%@ page import="com.bordozer.jphoto.ui.controllers.photos.groupoperations.PhotoGroupOperationModel" %>
<%@ page import="com.bordozer.jphoto.ui.controllers.photos.groupoperations.PhotoGroupOperationValidator" %>
<%@ page import="com.bordozer.jphoto.ui.controllers.photos.groupoperations.handlers.AbstractGroupOperationHandler" %>
...
<jsp:useBean id="photoGroupOperationModel" type="com.bordozer.jphoto.ui.controllers.photos.groupoperations.PhotoGroupOperationModel" scope="request"/>
        ...
        <form:form modelAttribute="photoGroupOperationModel" action="${eco:baseUrl()}/photos/groupOperations/confirm/">
            ...
            <div class="floatleft" style="padding-bottom: 30px;">
                ...
                <c:forEach var="photoGroupOperationEntry" items="${photoGroupOperationEntries}">
                    ...
                    <div class="block-border photo-container photo-container-${photo.id}"
                         style="position: relative; display: inline-block; vertical-align: top; min-height: 400px; height: auto; width: ${width}%; ${isGroupOperationAccessible ? "" : "border: 1px solid red;" } padding: 5px; margin: 5px;">

                        <div class="floatleft text-centered" style="height: auto; min-height: 320px; padding-bottom: 5px;"
                             onclick="toggleCheckbox( '${photo.id}' );">

                            <links:photoCard id="${photo.id}">
                                <img src="${photoGroupOperationEntry.photoPreviewImgUrl}" class="photo-preview-image" style="vertical-align: top;"
                                     title="${photo.nameEscaped}"/>
                            </links:photoCard>
                                ...
                                <c:if test="${photoGroupOperationEntryProperty.photoId == photo.id}">
                                    ...
                                    <div id="container-${entryId}">
                                        <input type="checkbox" id="${fieldId}" name="${fieldId}" value="true"
                                               class="group-operation-checkbox-${entryId} checkbox-${photo.id}"
                                               <c:if test="${photoGroupOperationEntryProperty.value}">checked</c:if> />
            ...
            <c:if test="${photoGroupOperationType == 'ARRANGE_TEAM_MEMBERS' or photoGroupOperationType == 'ARRANGE_PHOTO_ALBUMS'}">
                <script type="text/javascript">
