<%@ page import="com.bordozer.jphoto.core.general.user.EmailNotificationType" %>
<%@ page import="com.bordozer.jphoto.core.services.utils.UrlUtilsService" %>
<%@ page import="com.bordozer.jphoto.ui.context.ApplicationContextHelper" %>
<%@ page import="com.bordozer.jphoto.ui.controllers.users.notifications.UserNotificationsControlModel" %>
...
<jsp:useBean id="userNotificationsControlModel" class="com.bordozer.jphoto.ui.controllers.users.notifications.UserNotificationsControlModel"
             scope="request"/>

<%
    final UrlUtilsService urlUtilsService = ApplicationContextHelper.getBean(UrlUtilsService.BEAN_NAME);
%>
<c:set var="photosWithSubscribeOnNewCommentsLink"
       value="<%=urlUtilsService.getPhotosWithSubscribeOnNewCommentsLink( userNotificationsControlModel.getUser().getId() )%>"/>
<c:set var="usersNewPhotosNotificationMenuLink"
       value="<%=urlUtilsService.getUsersNewPhotosNotificationMenuLink( userNotificationsControlModel.getUser().getId() )%>"/>
...
<tags:page pageModel="${userNotificationsControlModel.pageModel}">
    ...
    <form:form modelAttribute="userNotificationsControlModel" method="POST">
        ...
        <table:table width="600">
            ...
            <table:tr>
                <table:td colspan="2">
                    <form:checkboxes path="${emailNotificationTypeIdsControl}" items="${emailNotificationTypes}" itemValue="id" itemLabel="name"
                                     htmlEscape="false" delimiter="<br />"/>
                </table:td>
            </table:tr>
