...
<button type="reset" id="resetButton" name="resetButton" class="ui-state-default ui-corner-all" title="${buttonTitle}"
        <c:if test="${not empty onclick}">onclick="${onclick}"></c:if> > &nbsp; ${buttonTitle}</button>
