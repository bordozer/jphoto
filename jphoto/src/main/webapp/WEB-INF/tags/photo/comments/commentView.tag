...
<%@ tag import="com.bordozer.jphoto.core.services.user.UserService" %>
...
<%@ tag import="com.bordozer.jphoto.ui.controllers.comment.edit.PhotoCommentModel" %>
...
<%@ attribute name="commentInfo" required="true" type="com.bordozer.jphoto.ui.controllers.comment.edit.PhotoCommentInfo" %>
...
<c:set var="anonymouslyPostedName"
       value="<%=ApplicationContextHelper.<UserService>getBean( UserService.BEAN_NAME ).getAnonymousUserName( EnvironmentContext.getLanguage() )%>"/>
                ...
                <h3 class="panel-title">
                        ...
                        <c:if test="${not isCommentOfPhotoAuthor}">
                            <c:if test="${commentReadTime == 0}">
                                <c:if test="${isThisPhotoOfCurrentUser}">
                                    <html:img id="" src="icons16/photoCommentNew.png" width="16" height="16"
                                              alt="${eco:translate('Comment View: New comment')}"/>
                                </c:if>

                                <c:if test="${not isThisPhotoOfCurrentUser}">
                                    <c:set var="readHint"
                                           value="${eco:translate1('Comment View: $1 has not read this comment yet', eco:escapeHtml(photoAuthor.name))}"/>
                    ...
                    <c:if test="${isCommentOfPhotoAuthor}">
                        <div class="col-lg-2">
                            ...
                            <html:img id="" src="icons16/photoCommentAuthor.png" width="16" height="16"
                                      alt="${eco:translate('Comment View: Photo_s author comment')}"/>
                        </div>
                    </c:if>
