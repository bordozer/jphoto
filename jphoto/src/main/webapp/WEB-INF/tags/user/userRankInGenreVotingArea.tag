...
<div id="user_by_genre_voting_${genre.id}">
    ...
    <c:if test="${! votingModel.validationResult.uiVotingIsInaccessible}">
        <c:if test="${not hasUserAlreadyVotedForThisGenre}">
            ...
            <a href="#" onclick="${jsFunctionVoteDown}"
               title="${eco:translate3('The member does not deserve his rank $1 (-$2) in category \'$3\'', userRankInGenre, loggedUserVotingPoints, genreNameTranslated)}">
                <html:img id="rank_down_${genre.id}" src="genre_rank_down.png" width="16" height="16"/>
            </a>

            &nbsp;

            <a href="#" onclick="${jsFunctionVoteUp}"
               title="${eco:translate3('The member deserves higher rank than his current one $1 (+$2) in category \'$3\'', userRankInGenre, loggedUserVotingPoints, genreNameTranslated)}">
    ...
    <c:if test="${votingModel.validationResult.uiVotingIsInaccessible}">
        ...
        <c:set var="errorMessage"
               value="${eco:translate1('You can not vote for the member rank in category $1:', genreNameTranslated)}<br /><br />${votingModel.validationResult.validationMessage}"/>
