...
import com.bordozer.jphoto.admin.jobs.loaders.AbstractSavedJobsLoader;
import com.bordozer.jphoto.admin.jobs.loaders.SavedJobLoaderFactory;
import com.bordozer.jphoto.admin.services.jobs.JobExecutionHistoryDaoImpl;
...
public class SavedJobListController {
    ...
    private void setActiveJobMaps(final SavedJobListModel model) {
        ...
        for (final JobExecutionHistoryEntry entry : historyEntriesOfActiveJobs) {
            ...
            final JobHistoryEntryDTO dto = new JobHistoryEntryDTO(entry.getId(), entry.getSavedJobType(), percentage);
            dto.setSavedJob(entry.getSavedJob() != null);

            activeJobHistoryEntries.put(entry.getId(), dto);
        }
